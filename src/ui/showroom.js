...
export function renderShowroomTab() {
        ...
        if (!slot.unlocked) {
            slotsHtml += `
                <div style="background:rgba(255,255,255,0.01); border:1px dashed var(--border-color); border-radius:10px; padding:2rem; text-align:center; opacity:0.6; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; min-height:160px">
                    ...
                    <p style="font-size:0.75rem; color:var(--text-muted)">Débloqué de manière permanente au Niveau ${slot.requiredLevel}</p>
                </div>
            `;
        } else if (pcItem === null) {
            slotsHtml += `
                <div style="background:rgba(255,255,255,0.02); border:1.5px dashed rgba(189,0,255,0.25); border-radius:10px; padding:2rem; text-align:center; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; min-height:160px">
                    ...
                    <div style="font-weight:700; font-size:0.95rem">Emplacement Libre n°${idx + 1}</div>
                ...
                <div class="part-card" style="padding:1.2rem; min-height:160px; display:flex; flex-direction:row; gap:1.5rem; justify-content:space-between; align-items:center; border-color:rgba(189,0,255,0.3)">
                    <div style="display:flex; flex-direction:column; gap:8px; flex:1">
                        <div style="display:flex; align-items:center; gap:10px">
                            <span class="status-badge on" style="background:rgba(189,0,255,0.15); color:var(--color-purple); font-size:0.6rem">En Exposition n°${idx + 1}</span>
                            <span class="part-price" style="font-size:1.15rem; color:var(--color-cyan)">★ ${pcItem.pc.score} Pts</span>
                        </div>
                        ...
                        <div style="font-size:0.75rem; color:var(--text-secondary); display:grid; grid-template-columns:1fr 1fr; gap:4px">
                            <div><strong>CPU :</strong> ${getComponentById(pcItem.pc.cpu?.partId)?.name || 'Aucun'}</div>
                            <div><strong>GPU :</strong> ${getComponentById(pcItem.pc.gpu?.partId)?.name || 'Aucun'}</div>
                            <div><strong>RAM :</strong> ${getComponentById(pcItem.pc.ram?.partId)?.name || 'Aucun'}</div>
                            <div><strong>Boîtier :</strong> ${getComponentById(pcItem.pc.case?.partId)?.name || 'Aucun'}</div>
                        </div>
                    </div>
        ...
        sponsorsHtml = `
            <div style="display:flex; flex-direction:column; gap:6px; font-size:0.8rem; color:var(--text-secondary)">
                ...
                <div style="display:flex; justify-content:space-between">
                    <span>Alimentations & Boîtiers</span><span class="text-emerald" style="font-weight:600">-${prestigeDiscountPercent}%</span>
                </div>
            </div>
        `;
    } else {
    ...
    prestigePanel.innerHTML = `
        ...
        <div style="background:rgba(0,0,0,0.15); border:1px solid var(--border-color); border-radius:8px; padding:12px; margin-bottom:1.5rem">
            ...
            <div style="font-size:2rem; font-weight:800; color:var(--color-cyan); font-family:var(--font-mono); text-shadow:0 0 10px rgba(0,240,255,0.25)">${totalPrestigeScore} Pts</div>
...
function openExposePCSelector(slotIdx) {
    ...
    header.innerHTML = `
        <h2>Sélectionner un PC pour la Vitrine ${slotIdx + 1}</h2>
        ...
        listHtml = `
            ...
            <div style="display:flex; flex-direction:column; gap:10px">
                ${options.map((opt, oIdx) => {
                    const cpu = getComponentById(opt.pc.cpu?.partId)?.name || 'Aucun';
                    const gpu = getComponentById(opt.pc.gpu?.partId)?.name || 'Aucun';
                    const srcLabel = opt.source === "workbench" ? `Établi ${opt.workbenchId}` : "Stock";
                    return `
                        <div class="part-card" style="padding:10px; flex-direction:row; justify-content:space-between; align-items:center; border-color:var(--border-color)">
                            <div style="display:flex; flex-direction:column; gap:3px">
                                ...
                                <div style="font-size:0.65rem; color:var(--color-purple)">Source : ${srcLabel}</div>
                            </div>
                            <div style="display:flex; flex-direction:column; align-items:flex-end; gap:5px">
                                <span class="text-cyan font-mono" style="font-weight:700; font-size:0.9rem">★ ${opt.score} Pts</span>
                                <button class="btn-buy" id="btn-select-expose-${oIdx}">Exposer</button>
                            </div>
                        </div>
                    `;
                }).join('')}
    ...
    options.forEach((opt, oIdx) => {
        document.getElementById(`btn-select-expose-${oIdx}`).addEventListener("click", () => {
            ...
            saveGame();
            overlay.remove();
            showToast(`PC "${opt.name}" placé en vitrine de prestige ! Vos remises en magasin augmentent !`, "success");
...
function retrievePCFromShowroom(slotIdx) {
    ...
    saveGame();
    showToast(`PC "${pcItem.name}" retiré du showroom et renvoyé dans votre inventaire.`, "info");
