...
export function renderInventoryTab() {
    ...
    headerRow.innerHTML = `
        ...
        <div style="display:flex; gap:8px" id="inventory-cat-buttons">
            <button class="btn-secondary ${inventoryCategory === 'all' ? 'text-cyan' : ''}" data-cat="all">Tout</button>
            ...
            <button class="btn-secondary ${inventoryCategory === 'storage' ? 'text-cyan' : ''}" data-cat="storage">💽 Stock</button>
            <button class="btn-secondary ${inventoryCategory === 'psu' ? 'text-cyan' : ''}" data-cat="psu">🔌 PSU</button>
            <button class="btn-secondary ${inventoryCategory === 'cooler' ? 'text-cyan' : ''}" data-cat="cooler">❄️ Vent</button>
        ...
        card.innerHTML = `
            ...
            <div>
                ...
                <div class="part-specs">
                    <div class="part-spec-item"><span>Caractéristiques :</span><span>${specLabel}</span></div>
                    <div class="part-spec-item"><span>Valeur achat :</span><span>${comp.price}$</span></div>
                </div>
            </div>
            
            <div class="part-price-row">
                <div class="part-price" style="font-size:0.95rem; color:var(--text-secondary)">Occase : ${sellValue}$</div>
...
function sellInventoryItem(item, val) {
    ...
    state.money += val;
    saveGame();
    showToast(`Pièce revendue pour ${val}$ !`, "success");
    window.updateHud();
    renderInventoryTab();
}
